...
4. 通过监听正确类型的DOM事件，等待操作完成。
5. 处理结果（结果可在request对象上取得）。

这个部分只给出简单的代码示例，用于快速上手，详细的各个对象的API放在后文介绍。
...
```js
const request = window.indexedDB.open(数据库名称, 数据库版本);
```

这个方法接收两个参数，第一个参数表示数据库的名称，第二个参数表示数据库的版本。这个版本同时可以做打开数据库和在没有指定名称数据库时创建数据库。第二个参数是可以省略，如果省略使用当前版本，创建数据库时，默认为1。

`indexedDB.open()`方法返回一个`IDBRequest`对象。这个对象通过三种事件`error`、`success`和`upgradeneeded`，处理打开数据库的操作结果。
...
request.onsuccess = (event) => {
    sonsole.log("打开或创建数据库成功", event.target.result); // 获取对象的DB数据库对象
}
```

#### upgradeneeded事件

如果指定的版本号，大于实际的版本号就会发生数据库升级事件，在创建数据库时也会触发，可以看做是版本`0 => 1`
...
主键（key）是默认建立索引的属性。比如数据记录是`{id: 1, name: "张三"}`，那么`id`属性可以作为主键，因为他有唯一性。主键也可以指定为下层对象的属性，比如`{foo: { id: 1 }}`的`foo.id`也可以指定为主键。

如果数据记录里面没有合适作为主键的属性，那么可以让`indexedDB`自动生成主键。

```js
const objectStore = db.createObjectStore("person", {
    autoIncrement: true, // 自动自增的值作为主键
})
```
...
request.upgradeneeded = (e) => {
    ...
    // 创建索引
    objectStore.createIndex("name", "name", { unique: false }); // name属性值可重复
    objectStore.createIndex("email", "email", { unique: true }); // email属性不可重复
}
```

上面代码中，`IDBObject.createIndex()`的三个参数分别为索引名称，索引所在属性和配置项（该属性值是否可以重复）。
...
[MDN IndexedDB指南](https://developer.mozilla.org/zh-CN/docs/Web/API/IndexedDB_API)

[网道 IndexedDB API](https://wangdoc.com/javascript/bom/indexeddb)